...
_COPYRIGHT_STR = '/** @copyright(c) 2021 Nio Inc. All rights reserved. \n\
 *  @brief get the around map of the vehicle. \n\
 *  @author gang yang, gang.yang1@nio.com \n\
 *  @date 2021/5/28 \n\
 * */\n\n'
...
def is_own_header(src_file, include):
  ...
  inc_pref = (os.path.basename(os.path.splitext(include)[0])).lower()
  src_pref = (os.path.basename(os.path.splitext(src_file)[0])).lower()
...
def sort_includes_batch(filename, includes):
  ...
  for name in sorted(includes.keys()):
    ...
    str_hfile = repr(hfile)
    by_type[inc_type].append(str_hfile)
  sorted_includes = []
  for sections in _SECTIONS_ORDER:
    for inc_type in by_type.keys():
      if inc_type in sections:
        sorted_includes.extend(['// ' + cpplint._IncludeState._TYPE_NAMES[inc_type]])
...
def sort_includes(filename, lines):
  ...
  includes_batches = 0
  in_batch = False
  multiline_comment_batches = 0
  int_multiline_comment = False
  multiline_comment = []
  includes = dict()
  new_lines = []
  find_copyright = False

  for lnum, line in enumerate(lines):
    line_strip = line.strip()
    if '/*' in line_strip:
      if not int_multiline_comment:
        multiline_comment_batches += 1
        int_multiline_comment = True
    if int_multiline_comment:
      if 'copyright' in line_strip:
        find_copyright = True
      multiline_comment.append(line_strip)
      if '*/' in line_strip:
        int_multiline_comment = False
    
    # 查找到之前的注释
    find_comment_before = False
    for val in cpplint._IncludeState._TYPE_NAMES.values():
      if val.strip() in line.strip():
        find_comment_before = True
    if find_comment_before:
      continue
  ...
  if not find_copyright:
    new_lines.insert(0, _COPYRIGHT_STR)
  return new_lines
